...
df = pd.read_csv('food_supply.csv', dtype={"id": str})
df_scatter = pd.read_csv('scatter_data.csv', dtype={"id": str})
clusters = pd.read_csv("final_clusters.csv")
box_cluster = pd.read_csv("box_cluster.csv")


response = open("european-union-countries.geojson")
...
app.layout = html.Div(
    [
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.H4(
                                    "European Countries Patter Regarding Food Consuption",
                                    style={"font-weight": "bold"},
                                ),
                                html.H5(
                                    "What We EAT VS How we Affect", style={"margin-top": "0px"}
                                ),
                            ]
                        )
                    ],
            ...
            [
                html.Div(
                    [
                        ...
                        html.H6("Food Consumption ", style={"margin-top": "0","font-weight": "bold","text-align": "center"}),

                        html.P(
                            "European countries has many similiarities but it's very different in Food habits." 
                            " The map illustrates the supply of food in KG per capita per year.",
                            className="control_label",style={"text-align": "justify"}
                        ),
                        html.P(),
                        html.P("Pick your favourite food", className="control_label",style={"text-align": "center","font-weight":"bold"}),
        ...
        html.Div(
            [
                html.H6("Health status among differnet european countries", style={"margin-top": "0","font-weight": "bold","text-align": "center"}),
                html.P("", className="control_label",style={"text-align": "justify"}),
        ...
        html.Div(
            [   
                html.Div(
                    ...
                    html.P("In this heatmap, we are finding the correlations between the five health variables and the eigtheen food variables.", className="control_label",style={"text-align": "justify"}),
                    #html.P("""<br>"""),
                ...
                html.Div(
                    [   html.H6("Analysing the correlations between food consumption and health", style={"margin-top": "0","font-weight": "bold","text-align": "center"}),
                        html.P("Box Plot And Viloin Plot", className="control_label",style={"text-align": "justify"}),
        ...
        html.Div(
            [
                    ...
                    html.P("Clustering is about to segregate groups with similar traits and assign them into clusters. Here we are doing it with food products and health variable.",className="control_label",style={"text-align": "justify"}),
                    html.Div([ 
                    html.P("Select a clustering choice", className="control_label",style={"text-align": "center","font-weight":"bold"}),
                    ...
                    html.P("Select a food value for cluster comparison", className="control_label",style={"text-align": "center","font-weight":"bold"}),
                    ots_behaviour,
                    dcc.Graph(id="boxes"),
                    ],className="pretty_container sixish columns",), 
...
def display_boxes(clust, vari):
    ...
    box_cluster = pd.read_csv("box_cluster.csv").rename(columns={"Food":"Food clusters", "Health":"Health clusters", "Food & Health":"Food & Health clusters"})
...
#running an app.py file will give us the dashboard running at local host .
